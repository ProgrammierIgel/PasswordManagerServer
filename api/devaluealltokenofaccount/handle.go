...
func Handle(store storage.Store) httprouter.Handle {
	return func(response http.ResponseWriter, request *http.Request, _ httprouter.Params) {
		...
		if err != nil {
			tools.WarningLog("Attempt to devalue all tokens from account. Cant read request.", err, request)
		...
		if err != nil {
			tools.WarningLog("Attempt to devalue all tokens from account. Cant unmarshal request.", err, request)
			http.Error(response, err.Error(), http.StatusInternalServerError)
			return
		}
		...
		if err != nil {
			tools.WarningLog("Attempt to devalue all tokens from account. Cant devalue token.", err, request)
			http.Error(response, err.Error(), http.StatusInternalServerError)
			return
		}
		...
		tools.DebugLog("Successfully all tokens of account devalued.", request)
