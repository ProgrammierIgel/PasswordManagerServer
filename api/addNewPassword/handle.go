...
func Handle(store storage.Store, logger *log.Logger) httprouter.Handle {
	return func(response http.ResponseWriter, request *http.Request, _ httprouter.Params) {
		...
		if err != nil {
			tools.WarningLog("Attempt to add new password. Cant read request.", err, request, logger)
		...
		if err != nil {
			tools.WarningLog("Attempt add new password. Cant unmarshal request.", err, request, logger)
			http.Error(response, err.Error(), http.StatusInternalServerError)
			return
		}
		...
		if err != nil {
			tools.WarningLog(fmt.Sprintf("Attempt to add new password (%s) on account. Cant add password.", requestBody.PasswordName), err, request, logger)
			http.Error(response, err.Error(), http.StatusInternalServerError)
			return
		}
		...
		tools.DebugLog(fmt.Sprintf("Added new password (%s) to account", requestBody.PasswordName), request, logger)
