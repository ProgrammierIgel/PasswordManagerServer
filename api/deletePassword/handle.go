...
func Handle(store storage.Store, logger *log.Logger) httprouter.Handle {
	return func(response http.ResponseWriter, request *http.Request, _ httprouter.Params) {
		...
		if err != nil {
			tools.WarningLog("Attempt to delete password. Cant read request.", err, request, logger)
		...
		if err != nil {
			tools.WarningLog("Attempt delete password. Cant unmarshal request.", err, request, logger)
			http.Error(response, err.Error(), http.StatusInternalServerError)
			return
		}
		...
		if err != nil {
			tools.WarningLog(fmt.Sprintf("Attempt to delete password (%s). Cant add password.", requestBody.PasswordName), err, request, logger)
			http.Error(response, err.Error(), http.StatusInternalServerError)
			return
		}
		...
		tools.DebugLog(fmt.Sprintf("Deleted password (%s)", requestBody.PasswordName), request, logger)
