...
func Handle(store storage.Store) httprouter.Handle {
	return func(response http.ResponseWriter, request *http.Request, _ httprouter.Params) {
		...
		if err != nil {
			tools.WarningLog("Attempt to devalue all tokens. Cant read request.", err, request)
		...
		if err != nil {
			tools.WarningLog("Attempt to devalue all tokens. Cant unmarshal request.", err, request)
			http.Error(response, err.Error(), http.StatusInternalServerError)
			return
		}
		...
		if err != nil {
			tools.WarningLog("Attempt to devalue all tokens. Cant devalue all.", err, request)
			http.Error(response, err.Error(), http.StatusInternalServerError)
			return
		}
		...
		tools.DebugLog("Successfully all tokens devalued.", request)
