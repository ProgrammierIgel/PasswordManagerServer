...
func New(path string, password string, logger *logger.Logger) *Store {
	...
	store.SyncFromFile()
	logger.Printf("[INFO]-[STORE] New Store was created")
	return store
}

func (s *Store) SyncFromFile() error {
	if s.syncDisabled {
		s.logger.Printf("[CRITICAL]-[STORE] syncronization is disabled")
		return nil
	}
	...
	if err != nil {
		s.logger.Printf("[CRITICAL]-[STORE] Secrets file was not found")
		return err
	}
	...
	if err != nil {
		s.logger.Printf("[CRITICAL]-[STORE] Content of secrets file is not compatible to JSON")
		return err
	}
	s.decryptionPasswords = secretsFile.MainPasswords
	s.secrets = secretsFile.Secrets
	s.logger.Printf("[INFO]-[STORE] System load save from file")
	return nil
}
...
func (s *Store) SyncToFile() error {
	...
	if err != nil {
		s.logger.Printf("[CRITICAL]-[STORE] Content cant saved to secrets file")
		return err
	}
	s.logger.Printf("[INFO]-[STORE] System saved to file")
	return nil

}

func (s *Store) AddNewAccount(account string, password string) error {
	salt, err := cryptography.GenerateSalt(15, s.logger)
	if err != nil {
		return err
	}
	s.SyncFromFile()
	if s.decryptionPasswords[account].PasswordHash != "" {
		s.logger.Printf("[WARN]-[STORE] Attempt to create a account that already exists (account name: %s)", account)
	...
	s.logger.Printf("[INFO]-[STORE] New Account was created with name %s", account)
	s.SyncToFile()
	return nil
}
...
func (s *Store) DeleteAccount(account string, token string) error {
	if !s.CheckToken(token) {
		s.logger.Printf("[WARN]-[STORE] Attemt to delete account %s account. Failed due to incorrect token.", account)
		return fmt.Errorf("incorrect token")
	}
	if s.token[token].AccountName != account {
		s.logger.Printf("[WARN]-[STORE] Attemt to delete account %s account. Failed due trying to delete other account", account)
	...
	s.logger.Printf("[WARN]-[STORE] Account %s was deleted", account)
	s.SyncToFile()
	return nil
}
...
func (s *Store) AddNewPassword(token string, passwordName string, passwordToAdd string, url string, username string) error {
	if !s.CheckToken(token) {
		s.logger.Printf("[WARN]-[STORE] Attemt to add a password (%s). Failed due to incorrect password.", passwordName)
		return fmt.Errorf("incorrect token")
	}
	tokenValue := s.token[token]

	if tools.IsElementInMap(passwordName, s.secrets[tokenValue.AccountName]) {
		s.logger.Printf("[WARN]-[STORE] Attemt to add a password (%s) on account %s but account already exists.", passwordName, tokenValue.AccountName)
		return fmt.Errorf("already exists")
	}
	hash, err := cryptography.Encrypt(passwordToAdd, tokenValue.MasterPassword, s.logger)
	if err != nil {
		s.logger.Printf("[CRITICAL]-[STORE] Cant hash password: %s", err.Error())
		return err
	}
	...
	s.logger.Printf("[INFO]-[STORE] New password (%s) added on account %s", passwordName, tokenValue.AccountName)
	s.SyncToFile()
	return nil
}
...
func (s *Store) DeletePassword(token string, passwordName string) error {
	if !s.CheckToken(token) {
		s.logger.Printf("[WARN]-[STORE] Attemt to delete password (%s) on account. Failed due to incorrect token.", passwordName)
		return fmt.Errorf("incorrect token")
	}
	tokenValue := s.token[token]

	if !tools.IsElementInMap(passwordName, s.secrets[tokenValue.AccountName]) {
		s.logger.Printf("[WARN]-[STORE] Attempt to deleting non-existing password")
	...
	s.logger.Printf("[WARN]-[STORE] Deleted %s password on account %s", passwordName, tokenValue.AccountName)
	s.SyncToFile()
	return nil
}
...
func (s *Store) GetPassword(token string, passwordName string) (string, error) {
	if !s.CheckToken(token) {
		s.logger.Printf("[WARN]-[STORE] Attemt to get password %s from account. Failed due to incorrect token to decryption.", passwordName)
		return "", fmt.Errorf("incorrect token")
	}
	tokenValue := s.token[token]
	if !tools.IsElementInMap(passwordName, s.secrets[tokenValue.AccountName]) {
		s.logger.Printf("[WARN]-[STORE] Attemt to get password %s from account %s but password doesn't exists on account", passwordName, tokenValue.AccountName)
		return "", fmt.Errorf("password on account not found")
	}
	defer s.logger.Printf("[INFO]-[STORE] Password to %s on account %s successfully returned", passwordName, tokenValue.AccountName)

	password, err := cryptography.Decrypt(s.secrets[tokenValue.AccountName][passwordName].Secret, tokenValue.MasterPassword)
	if err != nil {
		s.logger.Printf("[CRITICAL]-[STORE] Cant return password: %s", err.Error())
		return "", err
	}
	return password, nil
}
...
func (s *Store) GetURL(token string, passwordName string) (string, error) {
	if !s.CheckToken(token) {
		s.logger.Printf("[WARN]-[STORE] Attemt to get url %s from account. Failed due to incorrect token.", passwordName)
		return "", fmt.Errorf("incorrect token")
	}
	tokenValue := s.token[token]

	if !tools.IsElementInMap(passwordName, s.secrets[tokenValue.AccountName]) {
		s.logger.Printf("[WARN]-[STORE] Attemt to get url %s from account %s but url doesn't exists on account", passwordName, tokenValue.AccountName)
		return "", fmt.Errorf("url on account not found")
	}
	defer s.logger.Printf("[INFO]-[STORE] Url to %s on account %s successfully returned", passwordName, tokenValue.AccountName)
...
func (s *Store) GetUsername(token string, passwordName string) (string, error) {
	if !s.CheckToken(token) {
		s.logger.Printf("[WARN]-[STORE] Attemt to get username %s from account. Failed due to incorrect token.", passwordName)
		return "", fmt.Errorf("incorrect token")
	}
	tokenValue := s.token[token]

	if !tools.IsElementInMap(passwordName, s.secrets[tokenValue.AccountName]) {
		s.logger.Printf("[WARN]-[STORE] Attemt to get username %s from account %s but url doesn't exists on account", s.secrets[tokenValue.AccountName][passwordName].Username, tokenValue.AccountName)
		return "", fmt.Errorf("url on account not found")
	}
	defer s.logger.Printf("[INFO]-[STORE] Username to %s on account %s successfully returned", passwordName, tokenValue.AccountName)
...
func (s *Store) GetAllPasswordNamesOfAccount(token string) ([]string, error) {
	if !s.CheckToken(token) {
		s.logger.Printf("[WARN]-[STORE] Attemt to get all registered passwords from account. Failed due to incorrect token to decryption.")
	...
	s.logger.Printf("[INFO]-[STORE] All PasswordNames of Account %s returned", tokenValue.AccountName)
...
func (s *Store) DisableSync(password string) (bool, error) {
	...
	s.syncDisabled = true
	s.logger.Printf("[CRITICAL]-[STORE] !SYNC IS DISABLED!")
...
func (s *Store) EnableSync(password string) (bool, error) {
	...
	s.logger.Printf("[INFO]-[STORE] SYNC IS ENABLED")
	return s.syncDisabled, nil
}
...
func (s *Store) ChangeUsername(token string, passwordName string, newUsername string) error {
	if !s.CheckToken(token) {
		s.logger.Printf("[WARN]-[STORE] Attemt to change username to %s. Failed due to incorrect token.", newUsername)
		return fmt.Errorf("incorrect token")
	}
	tokenValue := s.token[token]

	if !tools.IsElementInMap(passwordName, s.secrets[tokenValue.AccountName]) {
		s.logger.Printf("[WARN]-[STORE] Attemt to change username %s from account %s but username doesn't exists on account", passwordName, tokenValue.AccountName)
		return fmt.Errorf("username on account not found")
	}
	defer s.logger.Printf("[INFO]-[STORE] URL to %s on account %s successfully changed", passwordName, tokenValue.AccountName)
...
func (s *Store) ChangeURL(token string, passwordName string, newURL string) error {
	if !s.CheckToken(token) {
		s.logger.Printf("[WARN]-[STORE] Attemt to change URL to %s. Failed due to incorrect password.", newURL)
		return fmt.Errorf("incorrect token")
	}

	tokenValue := s.token[token]
	if !tools.IsElementInMap(passwordName, s.secrets[tokenValue.AccountName]) {
		s.logger.Printf("[WARN]-[STORE] Attemt to change url %s from account %s but url doesn't exists on account", passwordName, tokenValue.AccountName)
		return fmt.Errorf("username on account not found")
	}
	defer s.logger.Printf("[INFO]-[STORE] URL to %s on account %s successfully changed", passwordName, tokenValue.AccountName)
	currentPasswordStruct := s.secrets[tokenValue.AccountName][passwordName]
...
func (s *Store) ChangePassword(token string, passwordName string, newSecret string) error {
	if !s.CheckToken(token) {
		s.logger.Printf("[WARN]-[STORE] Attemt to change password to %s. Failed due to incorrect token.", newSecret)
		return fmt.Errorf("incorrect token")
	}
	tokenValue := s.token[token]

	if !tools.IsElementInMap(passwordName, s.secrets[tokenValue.AccountName]) {
		s.logger.Printf("[WARN]-[STORE] Attemt to change password %s from account %s but url doesn't exists on account", passwordName, tokenValue.AccountName)
		return fmt.Errorf("username on account not found")
	}
	defer s.logger.Printf("[INFO]-[STORE] URL to %s on account %s successfully changed", passwordName, tokenValue.AccountName)

	hash, err := cryptography.Encrypt(newSecret, tokenValue.MasterPassword, s.logger)
	if err != nil {
		s.logger.Printf("[CRITICAL]-[STORE] Cant hash password: %s", err.Error())
		return err
	}
...
func (s *Store) ChangePasswordName(token string, passwordName string, newPasswordName string) error {
	if !s.CheckToken(token) {
		s.logger.Printf("[WARN]-[STORE] Attemt to change passwordname %s to %s. Failed due to incorrect token.", passwordName, newPasswordName)
		return fmt.Errorf("incorrect token")
	}
	tokenValue := s.token[token]

	if tools.IsElementInMap(newPasswordName, s.secrets[tokenValue.AccountName]) {
		s.logger.Printf("[WARN]-[STORE] Attemt to change passwordname %s from account %s but id already exists", passwordName, tokenValue.AccountName)
		return fmt.Errorf("new passwordname already exists (overwrite)")
	}

	if !tools.IsElementInMap(passwordName, s.secrets[tokenValue.AccountName]) {
		s.logger.Printf("[WARN]-[STORE] Attemt to change passwordname %s from account %s but but passwordname doesn't exists on account", passwordName, tokenValue.AccountName)
		return fmt.Errorf("passwordname on account not found")
	}
	defer s.logger.Printf("[INFO]-[STORE] Passwordname to %s on account %s successfully changed", passwordName, tokenValue.AccountName)
...
func (s *Store) CreateToken(accountName string, masterpassword string, remoteAddress string) (string, error) {
	...
	if err != nil {
		s.logger.Printf("[WARN]-[STORE] Attemt to create token on account %s. Failed due to incorrect password.", accountName)
		return "", err
	}
	...
	s.logger.Printf("[INFO]-[STORE] Successfully token on account %s created.", accountName)
	return newID, nil
...
func (s *Store) CheckToken(token string) bool {
	...
	if !tools.IsIDInMap(token, s.token) {
		s.logger.Printf("[INFO]-[STORE] token to check not registered")
		return false
	}
	oneHourInMillisec := int64(3600000)
	if currentTime.UnixMilli()-s.token[token].Timestamp.UnixMilli() > oneHourInMillisec {
		s.logger.Printf("[INFO]-[STORE] time of token to check is over")
...
func (s *Store) DevalueToken(token string) {
	...
	defer s.logger.Printf((fmt.Sprintf("[INFO]-[STORE] Successfully token from account %s devalued", s.token[token].AccountName)))
	s.token = tools.RemoveTokenFromMap(token, s.token)
}
...
func (s *Store) CheckPassword(account string, password string, remoteAddress string) error {
	...
	case accessess > 3:
		if currentTime.UnixMilli()-timestamp.UnixMilli() < time.Minute.Milliseconds()*5 {
			s.logger.Printf("[WARN]-[STORE] Too many tries to check password! Must wait!")
...
func (s *Store) DevalueAllTokens(password string) error {
	if password != s.password {
		s.logger.Printf("[WARN]-[STORE] Attempt to devalue all tokens. Failed due incorrect password")
		return fmt.Errorf("incorrect password")
	}
	// Path and Password are not needed to create new token object
	newStore := New("", "", s.logger)
	s.token = newStore.token
	s.logger.Printf("[WARN]-[STORE] All tokens succesfully devalued!")
	return nil
}

func (s *Store) DevalueAllTokensOfAccount(token string) error {
	if !s.CheckToken(token) {
		s.logger.Printf("[WARN]-[STORE] Attempt to devalue all tokens of account. Token incorrect")
	...
	s.logger.Printf("[INFO]-[STORE] Successfully all tokens of account %s devalued", accountName)
	return nil
}

func (s *Store) GetAllActiveTokens(token string) (uint, error) {
	if !s.CheckToken(token) {
		s.logger.Println("[WARN]-[STORE] Attemt to get the number of all active tokens. Failed due to incorrect token.")
	...
	s.logger.Printf("[INFO]-[STORE] Successuly number of all active tokens returned")
